...
(function() {
    ...
    function resize(e) {
        screenWidth  = gravityCanvas.width  = $("#gravity-demo-wrap").width();
        screenHeight = gravityCanvas.height = $("#gravity-demo-wrap").height();
        bufferCvs.width  = screenWidth;
        bufferCvs.height = screenHeight;
        context   = gravityCanvas.getContext('2d');
        bufferCtx = bufferCvs.getContext('2d');

        var cx = gravityCanvas.width * 0.5,
            cy = gravityCanvas.height * 0.5;
    ...
    function mouseMove(e) {
        let offsetLeft = gravityCanvas.getBoundingClientRect().left + $(window).scrollLeft();
        let offsetTop = gravityCanvas.getBoundingClientRect().top + $(window).scrollTop();
        ...
        gravityCanvas.style.cursor = hit ? 'pointer' : 'default';
    }
    ...
    function mouseDown(e) {
        ...
        console.log(x, y, " ", e.clientX, e.clientY, " ", gravityCanvas.width, gravityCanvas.height);
        console.log(e.pageY, offsetTop, gravityCanvas.getBoundingClientRect().top, $(window).scrollTop());
        console.log($("#gravity-demo-wrap").height());
    ...
    gravityCanvas.addEventListener('mousemove', mouseMove, false);
    gravityCanvas.addEventListener('mousedown', mouseDown, false);
    gravityCanvas.addEventListener('mouseup', mouseUp, false);
    gravityCanvas.addEventListener('dblclick', doubleClick, false);


    // GUI

    gui = new dat.GUI({ autoPlace: false });
    $("#gui-wrap").append($(gui.domElement));
    ...
    const GRAVITY_CUT_POINT = 27000;

    // Start Update
    // function onScroll() {
    //     if ($(window).scrollTop() < GRAVITY_CUT_POINT && $(window).scrollTop() > GRAVITY_CUT_POINT-2000){
    //       animateGravity();
    //     }
    //   } 
    var animateGravity = function() {
        var i, len, g, p;
        ...
        //     window.requestAnimationFrame(animateGravity());
        //   }
        window.requestAnimationFrame(animateGravity);
    };
    animateGravity();
